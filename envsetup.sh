...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var ORCA_VERSION)
        ZIPFILE=orca-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.orca.device=$ORCA_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $BF_BUILD, run away!"
    fi
}
